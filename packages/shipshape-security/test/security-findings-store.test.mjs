...
import {
  ...
  lastVerification,
} from '../src/core/security-findings-store.mjs';
import { triageFindings } from '../src/core/security-findings-triage.mjs';
import { validateSecurityFindings } from '../src/core/security-findings-check.mjs';
...
test('lastVerification returns newest by at', () => {
  const finding = {
    verifications: [
      { at: '2026-01-01T00:00:00.000Z', result: 'pass', method: 'old' },
      { at: '2026-06-01T00:00:00.000Z', result: 'fail', method: 'new' },
    ],
  };
  const last = lastVerification(finding);
  assert.equal(last.method, 'new');
