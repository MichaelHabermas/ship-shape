...
| --- | --- | --- | --- |
| Improvement Report | `my-docs/IMPROVEMENT_REPORT.md` | Ready | Ledger generated block shows Categories 1-8 `proven`; narrative sections link proof and claim boundaries. |
| Discovery Write-Up | `my-docs/discovery-research-log.md` | Ready | Contains more than three discoveries with file paths, dispositions, and future-application notes; stale architecture candidates are retired by the evidence-freeze entry. |
...
| Reviewer Evidence Bundle | `https://ship-shape-reviewer-evidence.onrender.com/` | Render static site | Generated on deploy by `pnpm submission:render-bundle`; local path `my-docs/reviewer-evidence-bundle/index.html`; service `ship-shape-reviewer-evidence`, no SPA rewrite. |
...
- Dashboard: `https://ship-shape-reviewer-evidence.onrender.com/my-docs/reviewer-dashboard.html`
...
| `DATABASE_URL=postgresql://ship:ship_dev_password@localhost:5432/ship_test_audit pnpm --filter @ship/api exec vitest run` | Blocked locally; escalated run reached host network but failed with `ECONNREFUSED` because PostgreSQL was not listening on `localhost:5432`. |
