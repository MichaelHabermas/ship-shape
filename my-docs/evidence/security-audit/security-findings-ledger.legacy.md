...
| Field | Value |
| --- | --- |
| Discovery date | 2026-05-22 (single deep authorization review session) |
| Method | Deep OWASP Top 10 code review + authorization logic tracing (agent-assisted) |
| ID assignment | SS-FIND-001…034 numbered in order as findings were confirmed in that session; **later IDs are not higher severity** — only later in the write-up |
| Probe state at discovery | `pnpm security:probe -- --run-id cat8-final` — 4/4 surfaces, 25/25 probes passed, 0 findings |
| Why probes missed these | Probes cover auth perimeter, CSRF, WebSocket framing, search leakage, file headers — not governance-field mass assignment or business-logic authorization on accountability workflows |
...
| `fixed` | Remediated — must link before/after evidence (probe run ID, test name, or commit) |
| `wont-fix` | Accepted risk with documented rationale |
...
| --- | --- | --- | --- | --- |
| SS-FIND-001 | Critical | open | Governance approval forgery via document `properties` mass assignment | `api/src/routes/documents.ts` |
| SS-FIND-002 | Critical | open | RACI self-elevation (`accountable_id`) grants real approval authority | `api/src/routes/programs.ts`, `documents.ts` |
| SS-FIND-003 | Critical | open | Week lifecycle bypass via ungoverned `status` PATCH | `api/src/routes/weeks/sprints.ts`, `documents.ts` |
| SS-FIND-004 | High | open | Weekly plan IDOR via generic documents REST API | `api/src/routes/documents.ts` vs `weekly-plans.ts` |
| SS-FIND-005 | High | open | Weekly plan IDOR via collaboration WebSocket | `api/src/collaboration/index.ts` |
| SS-FIND-006 | Medium | open | Collaboration audit trail attributes edits to document creator | `api/src/collaboration/index.ts` |
| SS-FIND-007 | Medium | open | Bulk issue move to private sprint/project without visibility check | `api/src/routes/issues.ts` |
| SS-FIND-008 | Medium | open | File access is workspace-scoped, not document-scoped | `api/src/routes/files.ts` |
| SS-FIND-009 | Medium | open | Private sub-issue metadata leaked in parent close warning | `api/src/routes/issues.ts` |
| SS-FIND-010 | Medium | open | Dashboard `my-focus` leaks private project/program metadata | `api/src/routes/dashboard.ts` |
| SS-FIND-011 | Medium | open | Team grid leaks private program metadata via visible issues | `api/src/routes/team.ts` |
| SS-FIND-012 | Medium | open | Public feedback endpoint lacks dedicated anti-abuse controls | `api/src/routes/feedback.ts` |
| SS-FIND-013 | Medium | open | S3 upload confirm without object verification | `api/src/routes/files.ts` |
| SS-FIND-014 | Medium | open | File extension blocklist checks last extension only | `api/src/routes/files.ts` |
| SS-FIND-015 | Medium | open | WebSocket session not re-validated after upgrade | `api/src/collaboration/index.ts` |
| SS-FIND-016 | Medium | open | API tokens inherit full user privileges (no route scoping) | `api/src/middleware/auth.ts` |
| SS-FIND-017 | Medium | open | Session IP/User-Agent stored but never enforced | `session-auth.ts`, `auth.ts` |
| SS-FIND-018 | Critical* | open | Setup race — first caller becomes super-admin if API exposed pre-bootstrap | `api/src/routes/setup.ts` |
| SS-FIND-019 | Low–Med | open | CAIA `returnTo` validation may allow open-redirect variants | `api/src/routes/caia-auth.ts` |
| SS-FIND-020 | Medium | open | Super-admin SSRF via CAIA `issuer_url` on credentials save | `admin-credentials.ts`, `services/caia.ts` |
| SS-FIND-021 | Low | open | Setup status endpoint reveals uninitialized instance | `api/src/routes/setup.ts` |
| SS-FIND-022 | Low | open | Accountability action-items may expose private sprint titles | `api/src/services/accountability.ts` |
| SS-FIND-023 | Low | open | AI analyze routes accept unvalidated request bodies | `api/src/routes/ai.ts` |
| SS-FIND-024 | Low | open | Document context breadcrumbs may expose hidden program UUID | `api/src/routes/associations.ts` |
| SS-FIND-025 | High | open | Pending file upload hijacking — complete endpoints ignore `uploaded_by` | `api/src/routes/files.ts` |
| SS-FIND-026 | High | open | Cross-site WebSocket session riding — no `Origin` check on upgrade | `api/src/collaboration/index.ts` |
| SS-FIND-027 | High | open | Unbounded in-memory Y.Doc cache — WS connection abuse / DoS | `api/src/collaboration/index.ts` |
| SS-FIND-028 | Medium | open | Invite accept creates sessions with `uuidv4()` not secure session IDs | `api/src/routes/invites.ts` |
| SS-FIND-029 | High | open | Super-admin cross-workspace access without membership / blanket admin | `workspaces.ts`, `document-access.ts` |
| SS-FIND-030 | Medium | open | Member can enable `public_feedback_enabled` via ungoverned PATCH | `api/src/routes/documents.ts`, `programs.ts` |
| SS-FIND-031 | Medium | open | Production `SameSite=None` — cross-site cookies; CSRF-only REST defense | `session-cookies.ts`, `app.ts` |
| SS-FIND-032 | Medium | open | Unauthenticated invite lookup leaks email and workspace metadata | `api/src/routes/invites.ts` |
| SS-FIND-033 | Medium | open | Any workspace member can mint full-privilege API tokens | `api/src/routes/api-tokens.ts` |
| SS-FIND-034 | Low | open | Full OpenAPI schema exposed without authentication | `api/src/swagger.ts` |

\* SS-FIND-018 is **Critical only** when the API is internet-reachable before legitimate first-time setup completes.

**ID assignment note:** Do **not** add duplicate IDs for the same bug. Cross-reference related entries instead (e.g. SS-FIND-016 ↔ SS-FIND-033). A mistaken duplicate file `findings-ledger.md` was removed; **this file is the only ledger.**

---

## Related finding clusters

Findings share root causes. Fix the cluster once where possible.

| Cluster | Findings | Shared root cause |
| --- | --- | --- |
| **Generic PATCH bypasses governance** | 001, 002, 003, 012, 030 | `PATCH /api/documents/:id` and related routes merge client `properties` without denylist; dedicated approval/lifecycle routes are bypassed |
| **Weekly plan/retro authorization split** | 004, 005, 006 | `/api/weekly-plans` enforces person ownership; generic REST + WebSocket use workspace visibility only |
| **File upload trust boundary** | 008, 013, 014, 025 | Files authorized by workspace, not uploader or parent document; confirm paths skip integrity checks |
| **WebSocket lifecycle & abuse** | 005, 015, 026, 027 | Upgrade auth exists but Origin, session re-validation, and resource caps are incomplete |
| **Private metadata via joins** | 009, 010, 011, 022, 024 | Queries join related documents without `VISIBILITY_FILTER_SQL` on every alias |
| **Credential surface** | 016, 017, 028, 033 | API tokens are full-privilege and any member can mint them; sessions store IP/UA but never bind; invite path uses weaker IDs |
| **Bootstrap / deploy exposure** | 018, 021, 034 | Unauthenticated setup/status/OpenAPI when API is reachable pre-bootstrap or for recon |
| **Super-admin break-glass** | 019, 020, 029 | CAIA SSRF/redirect paths; cross-workspace switch + blanket `isAdmin` without membership |
| **Cross-site session abuse** | 026, 031 | Production cookies sent cross-site; WS has no Origin check; REST relies entirely on CSRF middleware |
| **Invite surface** | 028, 032 | Weak session ID on accept; unauthenticated GET leaks invitee email and workspace name |
...
| **Severity** | Medium |
| **Status** | open |
| **OWASP** | A01 Broken Access Control (IDOR metadata) |
...
| **Cluster** | File upload trust boundary (008, 013, 014, 025) |
...
| **Cluster** | WebSocket lifecycle & abuse (005, 015, 026, 027) |
...
| **Cluster** | Credential surface (016, 017, 028) |
...
| **Cluster** | Super-admin break-glass (019, 020, 029) |
...
| **Cluster** | Generic PATCH bypasses governance (001, 002, 003, 012, 030) |
...
| **Cluster** | Cross-site session abuse (026, 031) |
...
| **OWASP** | A01 Broken Access Control |
| **Category** | Information disclosure |
| **Cluster** | Invite surface (028, 032) |
...
| **Cluster** | Credential surface (016, 017, 028, 033) |
...
| **Cluster** | Bootstrap / deploy exposure (018, 021, 034) |
