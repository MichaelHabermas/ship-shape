...
- Removed stale evidence-run archive directories; retained durable Category 2 values in the ledger and reports.
...
### Evidence
- `pnpm security:probe:ci` | pass; latest durable summary is `my-docs/evidence/security-audit/latest.json`
...
| 2 | Lazy lowlight subset | Largest chunk 837.24 KB (`PropertyRow-*`); total JS/CSS 2396.29 KB | Largest chunk 642.48 KB; total JS/CSS 2369.69 KB; entry 528.48 KB | Cat 2 ledger values and this report |
...
Production Vite build output plus `web/dist/assets` JS/CSS byte count. The closeout evidence uses the historical baseline treemap/stats plus dependency-free after bundle-analysis values recorded in this report and the submission ledger.
...
- `pnpm build:web`: pass.
- Latest easy-wins bundle stats are recorded in this report and `cat2-latest-bundle` in `my-docs/evidence/submission-ledger.json`.
- Prior closeout bundle stats were superseded by the easy-wins values above.
...
Baseline on unfixed main: probe v2 found 10 findings, all 10 known-open, matching the SS-FIND Phase 1–2 backlog. Historical perimeter closeout remains a separate scope concept.
...
- `pnpm security:probe:test`: pass (32/32)
- `pnpm security:probe:ci`: pass, 5/5 surfaces, 40 probes, 0 findings recorded in the security findings ledger
