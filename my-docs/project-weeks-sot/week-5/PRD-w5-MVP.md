...
**Problem Statement:** Ship records project work, but blocked issues can sit without the right person noticing in time. PMs should not have to poll dashboards to discover that execution is drifting.

**Proposed Solution:** FleetGraph detects visible issues with `state = blocked`, creates an action-ready finding, shows it through the left-rail notification surface, and lets the user discuss the finding in contextual chat without allowing FleetGraph to mutate Ship or contact anyone autonomously. "Action-ready" means FleetGraph prepares evidence, stakeholder role candidates when known, a safe next-action class, and human-gate metadata. It does not mean FleetGraph knows or executes the correct human decision.
...
- Each MVP finding includes a decision packet: visible evidence, summary, stakeholder role candidates when known, recommended next human decision/action class, proposed recipient rationale, uncertainty/missing-context notes, trace metadata, and a human gate.
...
- PM: primary user. Needs to notice blocked work early and decide the next unblock action.
...
- As a PM, I want FleetGraph to surface blocked work without being asked so that I can intervene before drift becomes invisible.
...
- A FleetGraph finding appears when a visible issue has `state = blocked` and no open finding already covers the same dedupe key.
- A lightweight proactive UI surface makes the finding noticeable from nearby navigation context, such as the left-rail notification list. MVP does not require a global FleetGraph inbox.
- The finding explains the issue, latest blocker signal when present, severity, confidence, recommended next human decision/action class, proposed smallest useful audience when known, and why that audience is smaller/better than the obvious broader audience.
- The finding may include an internal draft unblock message/action, but no UI should promise send/post/refine behavior until approved.
- The human gate records why approval is required before Ship mutation or communication. If send/post/mutate execution is not implemented in MVP, no PM-facing control should imply it will execute.
- On-demand "why was this flagged?" uses the current issue/week context and the existing finding state.
- Draft refinement and dismiss remain internal/API spare parts unless explicitly surfaced by a later product decision.
...
- The model receives bounded context only: source issue, current blocked state, latest blocker iteration when present, associated week/project context when available, assignee/owner display context, existing FleetGraph finding state, and permission-filtered evidence.

**Evaluation Strategy:**

- Candidate detection: visible issues with `issue.state = blocked` qualify. `issue_iterations.blockers_encountered` provides explanation/history for the block when present. Priority, active-week membership, owner, and non-empty blocker text are not eligibility gates.
- Evidence grounding: every user-visible claim must map to source issue/week/iteration evidence visible to the current user.
- Trace evidence: one proactive trace must create or update a finding; one on-demand trace must explain an existing finding. Preferred MVP evidence is three traces: proactive create, on-demand explain, and proactive duplicate/update or quiet exit.
...
- Safety: explanation and any future draft/dismiss actions must not mutate source issue/week records or contact people.
...
- At least two visible issues with `state = blocked`; at least one includes recent `issue_iterations.blockers_encountered` text explaining the block, and at least one proves missing blocker text does not suppress visibility.
...
- Issue that is not blocked.
- Blocked issue with no blocker iteration.
- Blocked medium/low issue.
...
- MVP requires one blocked-work detector, real Ship data, proactive visibility, contextual notification/chat UI, on-demand explain, a human gate, shared traces, and the timed 5-minute detection path.
...
- Broad on-demand prompts can create cost and privacy cliffs; MVP only explains existing contextual findings.
...
- Qualifying visible issue with `state = blocked` creates an open finding within 5 minutes. Missing blocker explanation creates a callout instead of suppressing the finding.
- Finding is discoverable from a lightweight proactive UI surface and can open the source issue.
...
- Draft refinement and dismiss remain non-surfaced until approved; if invoked internally, they must not mutate issue/week data.
