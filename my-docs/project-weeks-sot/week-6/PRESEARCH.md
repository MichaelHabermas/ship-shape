...
- Which OAuth flow does the agent use — Authorization Code, Device Grant, Client Credentials (RFC 6749 §4.4), or a delegated first-party token broker? Defend the choice.

  **Spec basis:** Plugforge requires the Part 2 agent to authenticate as a first-party OAuth app and consume the public API through the SDK. PRESEARCH explicitly asks whether the agent should use Authorization Code, Device Grant, or Client Credentials for first-party machine-to-machine. FleetGraph's existing boundary allows the agent to read permitted Ship state and keep FleetGraph-owned findings/runs/notifications internal, while Ship source-record mutations and external contact remain human-gated. The 2026-06-02 user decision locked delegated user-bound tokens for this slice.

  **Answer:** Use an internal delegated token broker for the first-party FleetGraph agent app, tied to the initiating user/session. Do not add Client Credentials for this slice: it would make CI deterministic, but it would also lose user-bound audit at the exact boundary Epic 7 is meant to prove. The broker mints short-lived real `oauth_access_tokens` for the first-party Ship Agent app, then the agent reads Ship source data through `@ship/sdk`, `/api/v1`, scopes, rate limits, and public audit logging. Scheduled no-user worker paths stay internal until a separate user/audit model is deliberately chosen.
...
- How is the agent's app seeded — at boot, via a migration, manually in dev? What guarantees it exists in deployed environments?
  ...
  **Answer:** Seed the first-party agent OAuth app with a numbered database migration plus idempotent service-time ensure. The app record has stable `client_id`, first-party flag, `system_key='ship-agent'`, and required read scopes. It does not need a raw client secret for this slice because access tokens are minted by the internal delegated broker, not by a Client Credentials token exchange. The broker creates normal `oauth_access_tokens` rows so public API validation and audit see a real app/user/token path.
...
- Which scopes does the agent request, and what is your defense for each? Does the agent need write scopes, or can it stay read-only behind a recommendation pattern?
  ...
  **Answer:** Keep the Ship Agent app read-only for this slice: `documents:read`, `issues:read`, and `sprints:read`. FleetGraph-owned findings/runs/notifications remain internal persistence, so there is no public FleetGraph write scope and no public finding/run write endpoint. Do not grant `documents:write`, `issues:write`, or `sprints:write` unless a later human-approved action flow explicitly requires them. Week 6 proof shows a first-party agent using the public API without privileged Ship source-record mutation.
