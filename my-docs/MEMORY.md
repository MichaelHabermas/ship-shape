...
- `pnpm openapi:check` is report-first because route/spec coverage is incomplete; use `pnpm openapi:check -- --strict` only when missing/stale route coverage is expected to block. Do not broadly migrate frontend API calls until the target route family is covered and not stale.
...
- Runtime response validation starts as test-time contract assertions with `expectOpenApiResponse`; production middleware remains deferred until the target route family has honest OpenAPI coverage, otherwise the app will validate against stale or missing contract shapes.
