...
- `pnpm a11y:closeout` runs the repeatable Playwright/axe closeout scan for `/docs`, the first `/documents/:id`, and `/my-week`, writing `test-results/a11y-closeout/axe-summary.json` plus screenshots. Add `-- --fail-on-serious` when the goal is a blocking gate. Current result: `/docs` 0, document page 1 serious `color-contrast` node on a small badge, `/my-week` 1 serious `color-contrast` violation across 10 nodes. Do not claim Category 7 complete until those are fixed and rerun.
