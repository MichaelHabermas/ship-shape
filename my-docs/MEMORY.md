...
- Submission evidence operating rule: if work changes any submission evidence, update `my-docs/evidence/submission-ledger.json` first. Schema v2 must cover canonical Categories 1-8, and `my-docs/reviewer-dashboard.html` is generated from that ledger rather than hand-edited. Keep claims honest: only mark `proven` when required rubric items and acceptance gates clear the source requirement; keep `partial`, `open`, `needs_fill_in`, or `not_measured` otherwise. Do not change unrelated categories. After ledger edits, run `pnpm submission:validate` and `pnpm submission:render`, then report changed ledger categories and any failing/warning acceptance tests.
