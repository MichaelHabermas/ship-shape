...
- Closeout axe probe (`test-results/a11y-closeout/axe-summary.json`) found 0 violations on `/docs` and `/documents/:id`, but `/my-week` still has 1 serious `color-contrast` violation across 13 nodes. Do not claim Category 7 complete until that is fixed and rerun.
