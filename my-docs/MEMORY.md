...
- Category 4 is proven by flow-level app-shell consolidation: old protected-docs startup fanout is 7 requests / 33 SQL queries; current `/api/bootstrap` is 1 request / 24 SQL queries. `scripts/explain-performance.mjs` now includes `before_after_bootstrap_explain`; do not restate this as a row-level N+1 fix or Cat 3 latency proof.
