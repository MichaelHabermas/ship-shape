...
- Documentation drift worth fixing before submission: align README local setup with `pnpm dev` + local PostgreSQL, remove/qualify instructions that tell operators to apply `api/src/db/schema.sql` to existing databases, and frame Render as the current public deployment while preserving AWS/Terraform docs as future/optional infrastructure.
...
- `pnpm a11y:closeout -- --fail-on-serious` is the Category 7 blocking gate for `/docs`, `/projects`, the first real `/documents/:id`, and supporting `/my-week`, writing `test-results/a11y-closeout/axe-summary.json` plus screenshots. As of 2026-05-22, it passes with 0 critical/serious violations on the source-backed top three plus the supporting My Week page.
...
- Category 2 closeout uses `scripts/evidence/collectors/bundle-stats.mjs` to generate both JSON stats and `bundle-analysis.html` from `web/dist`. This is enough after-artifact proof for initial-entry/code-splitting claims, but total JS/CSS can still increase and must stay a caveat.
- Category 4 is proven by flow-level app-shell consolidation: old protected-docs startup fanout is 7 requests / 33 SQL queries; current `/api/bootstrap` is 1 request / 24 SQL queries. `scripts/explain-performance.ts` (via `pnpm perf:explain`) includes `before_after_bootstrap_explain`; do not restate this as a row-level N+1 fix or Cat 3 latency proof.
