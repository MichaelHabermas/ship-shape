...
| Total Critical/Serious violations                              | Axe: 2 critical nodes and 40 serious nodes across scanned pages; Lighthouse: 6 page-failures across 4 unique issue types | `pnpm a11y:closeout -- --fail-on-serious`: `/login` 0, `/docs` 0, `/issues` 0, selected `/documents/:id` 0, `/projects` 0, supporting `/my-week` 0 | 2026-05-23    | Fixed on source-backed top three pages plus expanded closeout pages | Fix all Critical/Serious violations on explicitly chosen top 3 pages if using violation target |              |
...
- Repeatable closeout axe output: `pnpm a11y:closeout -- --fail-on-serious` writes `test-results/a11y-closeout/axe-summary.json` and screenshots for pre-login `/login`, `/docs`, `/issues`, `/projects`, a real `/documents/:id`, and supporting `/my-week`. Current output (2026-05-23): all six pages 0 critical/serious violations.
