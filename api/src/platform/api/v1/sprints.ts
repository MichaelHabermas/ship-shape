...
publicSprintsRouter.get(
  ...
  publicApiAsyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const parsed = PublicSprintListQuerySchema.safeParse(req.query);
    if (!parsed.success) {
...
publicSprintsRouter.get(
  ...
  publicApiAsyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const parsed = PublicSprintParamsSchema.safeParse(req.params);
    if (!parsed.success) {
      sendValidationError(req, res, parsed.error);
      return;
    }
    ...
    const sprint = await findPublicSprint(parsed.data.id, req.publicApi.userId, req.publicApi.workspaceId);
    if (!sprint) {
...
publicSprintsRouter.get(
  ...
  publicApiAsyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const params = PublicSprintParamsSchema.safeParse(req.params);
    const query = PublicSprintIssueListQuerySchema.safeParse(req.query);
    ...
    const sprint = await findPublicSprint(params.data.id, req.publicApi.userId, req.publicApi.workspaceId);
    if (!sprint) {
      req.publicApiErrorCode = 'not_found';
      sendPublicApiError(res, 404, {
        code: 'not_found',
...
async function listPublicSprintsPage(input: {
  userId: string;
  workspaceId: string;
  limit: number | undefined;
  cursor: PublicCursorPayload | null;
}): Promise<{ data: PublicSprint[]; next_cursor: string | null }> {
  const actor = {
    userId: input.userId,
...
async function findPublicSprint(
  id: string,
  userId: string,
  workspaceId: string
...
function publicSprintFromRow(row: PublicSprintRow): PublicSprint {
